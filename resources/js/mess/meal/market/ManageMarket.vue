    ...
    <div>
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header bg-primary  white">
                        <h4 class="modal-title"> <b>Add Market Info</b></h4>
                     ...
                     <form method="POST"  enctype="multipart/form-data" v-on:submit.prevent="store">
                        <div class="modal-body">
                            <div class="row">
                                ...
                                <div class="form-group col-md-6" :class="{'has-danger': (errors.hasOwnProperty('cost'))}">
                                    <label><b>Cost</b><span class="requiredField">*</span></label>
                                    <input class="form-control" v-model="form.cost">
                                    <div class="requiredField" v-if="(errors.hasOwnProperty('cost'))">{{ (errors.hasOwnProperty('cost')) ? errors.cost[0] :'' }}</div>
                                </div>
        ...
        <div class="modal fade text-left" id="editModel">
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header bg-primary  white">
                        <h4 class="modal-title"> <b>Edit Market info</b></h4>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
...
<script>
    ...
    export default {
        ...
        methods:{
            store:function(){
                axios.post(base_url+'market', this.form).then( (response) => {
                    $('#addModel').modal('hide');
            ...
            update:function (id) {
                axios.put(base_url+'market/'+id, this.form).then( (response) => {
        ...
        mounted(){
           ...
           $('#addModel,#editModel').on('hidden.bs.modal', function(){
            ...
            EventBus.$on('update-button-clicked', (id) => {
                ...
                axios.get(base_url+'market/'+id+'/edit').then((response) => {
